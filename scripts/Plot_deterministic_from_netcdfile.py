...
import sys
sys.path.append("/storage/homefs/ge24z347/Liestal_event/src")

from flow_depth_plotting import plot_water_depths_deterministic_from_netcdfile
...
def parse_extent(s: str):
    ...
    try:
        ...
        if len(parts) != 4:
            raise ValueError
        return tuple(parts)  # (xmin, xmax, ymin, ymax)
    except Exception:
...
def main():
    p = argparse.ArgumentParser(
        description="Plot deterministic water_depth PNGs from a NetCDF (no realization dim)."
    )
    ...
    p.add_argument("--threshold", type=float, default=0.01, help="Depth mask threshold (m)")
    p.add_argument("--vmax", type=float, default=2.0, help="Colorbar vmax (m)")
    p.add_argument("--case-label", default="Zell", help="Title prefix / case label")
    p.add_argument("--init-time", default=None, help="Init time string used in title (e.g. 2022-05-05T08:00:00)")
    ...
    p.add_argument("--extent-units", choices=["m", "km", "auto"], default="auto",
                   help="Units for --extent values (default: auto)")
    ...
    plot_water_depths_deterministic_from_netcdfile(
