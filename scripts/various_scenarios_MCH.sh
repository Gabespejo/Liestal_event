...
if [ "$#" -ne 5 ]; then
    ...
    echo "Example: $0 Zell_2m_v2 20 100 5 acc-gpu"
...
base_par="${case_folder}/${case_study}.par"

cd "$case_folder" || { echo "Error: Directory $case_folder not found!"; exit 1; }

if [ ! -f "$base_par" ]; then
    echo "Error: Base PAR file not found: $base_par"
    exit 1
fi
...
case $solver in
    acc|acc-gpu|fv1|fv1-gpu|dg2|dg2-gpu)
        ;;
    *)
...
prepare_temp_par() {
    ...
    # Remove old solver lines if they exist
    ...
    # Replace dynamicrainfile, dirroot, resroot
    sed -i "s|^dynamicrainfile.*$|dynamicrainfile           ${rain_file}|" "$output_file"
    sed -i "s|^dirroot.*$|dirroot                   ${output_root}|" "$output_file"
    sed -i "s|^resroot.*$|resroot                   ${output_root}|" "$output_file"

    # Insert solver flags just before sim_time
    local sim_line
    sim_line=$(grep -n "^sim_time" "$output_file" | cut -d: -f1)

    if [[ -z "$sim_line" ]]; then
        echo "Warning: sim_time not found in $output_file. Solver lines not inserted."
        return
    fi

    local insert_line=$((sim_line - 1))
...
for precip in $(seq "$start_precip" "$step_precip" "$end_precip"); do
    ...
    rain_file="${case_study}_${precip}.nc"
    output_dir="${case_study}_${precip}_${output_suffix}"
    temp_par="${case_folder}/tmp_${case_study}_${precip}_${solver}.par"

    if [ ! -f "$rain_file" ]; then
        echo "[WARN] Missing rain file: $rain_file -- skipping"
    ...
    echo "[INFO] Running scenario ${precip}"
    echo "       rain file : $rain_file"
    echo "       output dir: $output_dir"
    echo "       temp par  : $temp_par"

    prepare_temp_par "$base_par" "$temp_par" "$solver" "$rain_file" "$output_dir"

    echo "[DEBUG] Key lines in ${temp_par}:"
    grep -nE "^(dynamicrainfile|dirroot|resroot|acceleration|fv1|dg2|cuda|sim_time)" "$temp_par"

    "$lisflood_build/lisflood" "$temp_par"
    exit_code=$?

    if [ $exit_code -eq 0 ]; then
        echo "[INFO] Completed scenario ${precip} with solver ${solver}"
        runs_done=$((runs_done + 1))
    else
        echo "[ERROR] LISFLOOD failed for scenario ${precip} with EXIT_CODE=${exit_code}"
...
echo "Total: ${runs_total}, Done: ${runs_done}, Skipped: ${runs_skipped}, Failed: ${runs_failed}"
